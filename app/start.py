...
async def choice_language(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        if callback.data == "uz":
            ...
            # await bot.send_message(callback.from_user.id, uz_phone, reply_markup=phone_en())
            await bot.send_message(callback.from_user.id, uz_phone, parse_mode='markdown', reply_markup=phone_en())
